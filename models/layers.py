...
class ResBlock_point(nn.Module):
    def __init__(self, in_channel, out_channel, norm=False):
        super(ResBlock_point, self).__init__()
        self.main = nn.Sequential(
            BasicConv1d(in_channel, out_channel, kernel_size=1, stride=1, relu=True, norm=norm),
            BasicConv1d(out_channel, out_channel, kernel_size=1, stride=1, relu=False, norm=norm)
        )
